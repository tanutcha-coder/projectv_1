...
<body>
    ...
    <div class="container">
        ...
        <div class="row ">
            ...
            <div class="col">
                <h1 class="mt-5">New RT-PCR Page</h1><br>
                <form method="get" action="">
                    <div class="mb-3">
                        <label for="rtpcrtesting_ID" class="form-label">หมายเลขการตรวจRT-PCR (rtpcrtesting_ID)</label>
                        <input type="varchar" class="form-control" name="rtpcrtesting_ID" required placeholder="RTXXX">
                    </div>
                    <div class="mb-3">
                        <label for="rtpcr_datetime" class="form-label">วันเวลาที่ตรวจ (DateTime)</label>
                        <input type="datetime-local" class="form-control" name="rtpcr_datetime">
                    </div>

                    <div class="mb-3">
                        <label for="TestingID" class="form-label">หมายเลขID การตรวจATK (TestingID)</label>
                        <select name="TestingID" value="ID">
                            <?php foreach ($ATK_list as $cus) {
                                echo "<option value='$cus->TestingID'>$cus->TestingID</option>";
                            } ?>
                        </select><br>
                        <div class="mb-3">
                            <label for="officer_ID" class="form-label">ชื่อพนักงานส่งสารคัดหลั่ง (officer_ID)</label>
                            
                            <select name="officer_ID">
                                <?php foreach ($officer_list as $em) {
                                    echo "<option value='$em->officer_ID'>$em->officer_N</option>";
                                }  ?>
                            </select><br>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="labroom_booking_ID">
                                <?php foreach ($labroom_list as $cus) {
                                    echo "<option value='$cus->labroom_booking_ID'>$cus->labroom_booking_ID</option>";
                                } ?>
                            </select><br>
                        </div>
                        ...
                        <div class="d-grid gap-3">
                            <button class="w-100 btn btn-success btn-lg" type="submit" name="action" value="addRTPCR">Save</button>
